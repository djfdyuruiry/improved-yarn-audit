...
testExitCode() {
  expectedExitCode="$2"
  actualExitCode="$3"
  test="When ${1} then exit code is ${expectedExitCode}"
...
runTests() {
  echo -e "\e[1;33mOLD Tests 1-16 skipped, tech refresh needed to add proper yarn audit mock\e[0m"
  ...
  # test 18
  expectedVersion="$(printf '%s' "$(grep '"version": ' "${scriptPath}/../package.json" | cut -d '"' -f 4)")"
  ...
  # test 20
  callIya -d
  testExitCode "--debug is passed" "0 " "$?"
  ...
  callIya
  testExitCode ".iyarc contains no exclusions" "0" "$?"
